...
/*********************************************************************
 **  Function: setCC1101Register
 **  Directly write to a CC1101 register for advanced control
 *********************************************************************/
void setCC1101Register(uint8_t reg, uint8_t value) {
    if (bruceConfigPins.rfModule == CC1101_SPI_MODULE) {
...
/*********************************************************************
 **  Function: setCC1101Preamble
 **  Configure CC1101 preamble length (MDMCFG1: NUM_PREAMBLE)
 *********************************************************************/
void setCC1101Preamble(uint8_t length) {
    // NUM_PREAMBLE is in MDMCFG1 (0x13) bits 4-6
    // 0=2 bytes, 1=3, 2=4, 3=6, 4=8, 5=12, 6=16, 7=24
    if (bruceConfigPins.rfModule == CC1101_SPI_MODULE) {
        uint8_t mdmcfg1 = ELECHOUSE_cc1101.SpiReadReg(0x13);
        mdmcfg1 = (mdmcfg1 & 0x8F) | ((length & 0x07) << 4);
        ELECHOUSE_cc1101.SpiWriteReg(0x13, mdmcfg1);
    }
}

/*********************************************************************
 **  Function: setCC1101SyncWord
 **  Configure CC1101 sync word (SYNC1, SYNC0)
 *********************************************************************/
void setCC1101SyncWord(uint16_t syncWord) {
    if (bruceConfigPins.rfModule == CC1101_SPI_MODULE) {
        ELECHOUSE_cc1101.SpiWriteReg(0x04, (syncWord >> 8) & 0xFF);
        ELECHOUSE_cc1101.SpiWriteReg(0x05, syncWord & 0xFF);
    }
}

/*********************************************************************
 **  Function: rampFrequency
 **  Implement frequency agility by ramping frequency over a range
 *********************************************************************/
void rampFrequency(float start, float end, float step, uint32_t delayMs) {
    if (bruceConfigPins.rfModule != CC1101_SPI_MODULE) return;
    
    float current = start;
    bool direction = (end > start);
    
    while ((direction && current <= end) || (!direction && current >= end)) {
        setMHZ(current);
        vTaskDelay(pdMS_TO_TICKS(delayMs));
        if (direction) current += step;
        else current -= step;
